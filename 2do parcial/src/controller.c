...
int postMasPopular(LinkedList* list,char* path){
	...
	if(path != NULL && list != NULL){
		...
		sprintf(idStr, "%d", postMasLikeado->id);

		sprintf(disLikesStr, "%d",postMasLikeado->dislikes);

		sprintf(likesStr, "%d", postMasLikeado->likes);

		sprintf(followersStr, "%d",postMasLikeado-> followers);

		fprintf(f, "%s,%s,%s,%s,%s\n", idStr, postMasLikeado->user, likesStr, disLikesStr,followersStr);
